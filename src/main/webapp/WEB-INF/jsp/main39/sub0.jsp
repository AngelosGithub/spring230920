        ...
        function ajax4() {
            axios.get("main39/sub3")
                .catch((error) => {
                    let status = error.response.status;
                    if (status >= 400 && status < 500) {
                        document.getElementById('result1').innerHTML="요청이 잘못됨";
                    } else if (status >= 500 && status <= 599) {
...
<div>
    ...
    <div>
        <textarea id="notesTextArea" placeholder="notes" cols="30" rows="10"></textarea>
    </div>
    <div>
        <button onclick="ajax7AddEmployee()">button7</button>
    </div>
    <div>
        ...
        <script>
            function ajax7AddEmployee() {
                ...
                axios.post("/main39/sub6", {
                    ...
                    .then(() => {
                        document.getElementById("result2").textContent="등록성공";
                    })
                    .catch((error) => {
                        const code = error.response.status;
                        if (400<= code && code <500) {
                            document.getElementById("result2").textContent="잘못입력";
                        } else if (500 <= code) {
                            document.getElementById("result2").textContent="서버오류";
                        }
                    })
                    .finally(() => {
